...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("$10.5 $BNB", Decimal("10.5"), "BNB"),
        ("10,5$ BNB", Decimal("10.5"), "BNB"),
        ("0.1$ BNB", Decimal("0.1"), "BNB"),
        ("$0.1 BNB", Decimal("0.1"), "BNB"),
        ("1k$ BNB", Decimal("1000"), "BNB"),
        ("$1k BNB", Decimal("1000"), "BNB"),
        ("0.0001k$ BNB", Decimal("0.1000"), "BNB"),
...
@pytest.mark.parametrize(
    ("text", "expected_amount", "expected_ticker"),
    [
        ("0.09$ BNB", Decimal("0.09"), "BNB"),
        ("$0.09 BNB", Decimal("0.09"), "BNB"),
        ("0.00009k$ BNB", Decimal("0.09000"), "BNB"),
        ("0$ BNB", Decimal("0"), "BNB"),
    ],
)
def test_parse_low_fiat_to_crypto_amount(
    text: str,
    expected_amount: Decimal,
    expected_ticker: str,
) -> None:
    ...
    assert isinstance(parsed_conversion, ParsedLowFiatToCryptoAmount)
    assert parsed_conversion.usd_amount == expected_amount
    assert parsed_conversion.ticker == expected_ticker
    assert parsed_conversion.matched_text == text
